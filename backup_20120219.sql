...
CREATE  TABLE IF NOT EXISTS `topspejder`.`menuitems` (
  ...
  `fk_userright_id` INT(11) NULL ,
  `fk_organisation_id` INT(11) NULL ,
  PRIMARY KEY (`menuItemId`) ,
  UNIQUE INDEX `menuItemId` (`menuItemId` ASC) ,
  INDEX `fk_menuItems_userrights` (`fk_userright_id` ASC) ,
  INDEX `fk_organisation_id` (`fk_organisation_id` ASC) ,
  ...
  CONSTRAINT `fk_organisation_id`
    FOREIGN KEY (`fk_organisation_id` )
    REFERENCES `topspejder`.`organisation` (`organisationId` )
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB
...
CREATE  TABLE IF NOT EXISTS `topspejder`.`menus-menuitems-map` (
  `menusMenuItemsMapId` INT(11) NOT NULL ,
  ...
  INDEX `fk_parentItemId` (`fk_parentItemId` ASC) ,
  ...
  CONSTRAINT `fk_menuitems_menus-menuitems-map`
    ...
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_parentItemId`
    FOREIGN KEY (`fk_parentItemId` )
    REFERENCES `topspejder`.`menus-menuitems-map` (`menusMenuItemsMapId` )
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB
COMMENT = '					';
